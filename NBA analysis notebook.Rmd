...
---
g the *Run* button within the chunk or by placing your cursor inside it and pressing *Ctrl+Shift+Enter*. 


# Unsupervised Machine Learning project to study NBA player similarity- 
## Nadir Nibras


# initializing script and loading packages, data
...
o=corrplot(cor(mainData),method='number',  tl.cex = 0.4)
```
...
ggplot(statsDF,aes(statsDF$X1, statsDF$X2)) +
  ...
  geom_text_repel(data=mergedData, aes(label=Player), size=1.5+mergedData$VORP/max(mergedData$VORP))
...
fviz_dend(res.hc, k = 8, # Cut in 8 groups
          ...
          ,labelsize= 1, repel= TRUE,
          labels_track_height=5,lwd=0.2
                    )
```
